...
/* TODO: make a VALUE to nid function */
    ...
    p12 = PKCS12_create((char*)passphrase, (char*)friendlyname, key, x509, x509s,
...
ossl_pkcs12_initialize(int argc, VALUE *argv, VALUE self)
{
    ...
    int st = 0;
    PKCS12 *pkcs = rb_rdata_fetch(self);

    if(rb_scan_args(argc, argv, "02", &arg, &pass) == 0) return self;
    passphrase = NIL_P(pass) ? NULL : StringValuePtr(pass);
    in = ossl_obj2bio(arg);
    d2i_PKCS12_bio(in, &pkcs);
    rb_rdata_store(self, pkcs);
    BIO_free(in);

    pkey = cert = ca = Qnil;
    if(!PKCS12_parse(pkcs, (char*)passphrase, &key, &x509, &x509s))
