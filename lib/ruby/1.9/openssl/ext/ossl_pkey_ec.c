...
static VALUE ossl_ec_key_initialize(int argc, VALUE *argv, VALUE self)
{
    ...
    if (NIL_P(arg)) {
	    ...
	    ec = PEM_read_bio_ECPrivateKey(in, NULL, ossl_pem_passwd_cb, (char*)passwd);
            if (!ec) {
		OSSL_BIO_reset(in);
		ec = PEM_read_bio_EC_PUBKEY(in, NULL, ossl_pem_passwd_cb, (char*)passwd);
            }
            if (!ec) {
		OSSL_BIO_reset(in);
...
static VALUE ossl_ec_key_to_string(VALUE self, VALUE ciph, VALUE pass, int format)
{
    ...
    case EXPORT_PEM:
		...
		cipher = NULL;
            ...
            i = PEM_write_bio_ECPrivateKey(out, ec, cipher, NULL, 0, NULL, (char*)password);
    	} else {
...
{
    if (rb_respond_to(klass, rb_intern("copy")))
