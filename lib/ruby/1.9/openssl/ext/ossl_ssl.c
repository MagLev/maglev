...
ossl_sslctx_setup(VALUE self)
{
    ...
    X509_STORE *store;
    EVP_PKEY *key = NULL;
    const char *ca_path = NULL, *ca_file = NULL;
    int i, verify_mode;
    VALUE val;
...
ossl_ssl_initialize(int argc, VALUE *argv, VALUE self)
{
    ...
    if (!rb_obj_is_kind_of_(io, rb_cIO)) {
    	rb_raise(rb_eTypeError, "wrong argument (%s)! (Expected kind of IO)",
    		 rb_class2name(rb_class_of(io)));
    }
...
ossl_start_ssl(VALUE self, int (*func)(), const char *funcname, int nonblock)
{
	...
	case SSL_ERROR_WANT_WRITE:
            ...
            rb_io_wait_writable_(io);
            continue;
	case SSL_ERROR_WANT_READ:
            read_would_block(nonblock);
            rb_io_wait_readable_(io);
            continue;
