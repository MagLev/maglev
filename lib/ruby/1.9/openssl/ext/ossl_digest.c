...
#define SetDigestName(obj, ctx) \
    rb_iv_set((obj), "@_digestname", rb_str_new2(EVP_MD_name(EVP_MD_CTX_md(ctx))))
#define GetDigestName(obj) \
    rb_iv_get((obj), "@_digestname")
#define SetData(obj, val) \
    rb_iv_set((obj), "@_data", (val))
#define ResetData(obj) \
    rb_iv_set((obj), "@_data", rb_str_new("", 0))
#define GetData(obj) \
    rb_iv_get((obj), "@_data")
#define SetDigest(obj, ctx) \
    rb_iv_set(obj, "@_ctx", Data_Wrap_Struct(rb_cObject, 0, EVP_MD_CTX_destroy, ctx))
#define GetDigest(obj, ctx) do { \
    Data_Get_Struct(rb_iv_get((obj), "@_ctx"), EVP_MD_CTX, (ctx)); \
    if (!(ctx)) { \
...
ossl_digest_copy(VALUE self, VALUE other)
{
    ...
    SafeGetDigest(other, ctx);
    argv[0] = GetDigestName(other);
    argv[1] = GetData(other);
    ossl_digest_initialize(2, argv, self);
...
ossl_digest_update(VALUE self, VALUE data)
{
    ...
    rb_str_append(GetData(self), data);
    StringValue(data);
    GetDigest(self, ctx);
...
ossl_digest_finish(int argc, VALUE *argv, VALUE self)
{
    ...
    GetDigest(self, ctx);
    len = EVP_MD_CTX_size(ctx);
    result = (unsigned char*)xcalloc(sizeof(char), len);
