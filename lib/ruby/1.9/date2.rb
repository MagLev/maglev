...
#   def initialize(d=1) @d = d <=> 0 end  # in first opening
#
#   def d() @d end
#
#   protected :d
    ...
    def <=> (other)
      if other._kind_of?( Infinity) ; return d <=> other.d
      elsif other._isNumeric; return d
      else
  begin
  ...
  # ITALY     = 2299161 # 1582-10-15  # in first opening
...
"warning: do not use non-numerical object as julian day number anymore")
  ...
  def self.complete_frags(elem) # :nodoc:
   ...
   [nil, [:year, :wnum1, :cwday, :hour, :min, :sec]]].
    ...
    if g && g[0] && (g[1].size - g[2].size) != 0
      ...
      g_sym = g[0]
      #when :ordinal
      if g_sym._equal?( :ordinal )
  elem[:year] ||= d.year
  elem[:yday] ||= 1
      # when :civil
      elsif g_sym._equal?( :civil )
  g[1].each do |e|
    break if elem[e]
    elem[e] = d.__send__(e)
  end
  ...
  elem[:cwday] ||= 1
      # when :wday
      elsif g_sym._equal?( :wday )
  elem[:jd] ||= (d - d.wday + elem[:wday]).jd
      # when :wnum0
      elsif g_sym._equal?( :wnum0 )
  g[1].each do |e|
    break if elem[e]
    elem[e] = d.__send__(e)
  end
  ...
  class << self
    ...
    def once(*ids) # :nodoc:
      for id in ids
  module_eval <<-"end;"
# puts "in once, id = #{id.to_i}  #{id.to_s}"   # ask Peter
  ...
  # def initialize(ajd=0, of=0, sg=ITALY) @ajd, @of, @sg = ajd, of, sg end # in first opening
...
=end
  ...
  def + (n)
    if n._isNumeric; return self.class.new!(@ajd + n, @of, @sg)
    end
  ...
  def - (x)
    if x._isNumeric; return self.class.new!(@ajd - x, @of, @sg)
    elsif x._kind_of?( Date) ;    return @ajd - x.ajd
    end
  ...
  def <=> (other)
    if other._isNumeric; return @ajd <=> other
    elsif other._kind_of?( Date);    return @ajd <=> other.ajd
    end
    nil
  end
  ...
  def === (other)
    if other._isNumeric; return jd == other
    elsif other._kind_of?(Date);    return jd == other.jd
    end
    false
  end
...
#  :minute, :second, :second_fraction
  ...
  # GemStone: Workaround for Trac 701
  def offset
    super
  end
