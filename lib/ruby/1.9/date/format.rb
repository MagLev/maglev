...
# Maglev edits for performance optimization
...
class Date
  ...
  module Format # :nodoc:
    ...
    MONTHS_keys_or = MONTHS.keys.join('|').freeze
    ...
    DAYS_keys_or = DAYS.keys.join('|').freeze
    ...
    ABBR_MONTHS_keys_or = ABBR_MONTHS.keys.join('|').freeze
    ...
    ABBR_DAYS_keys_or = ABBR_DAYS.keys.join('|').freeze
  ...
  DAYS_keys_strptime = ( /\A(#{Format::DAYS_keys_or})/io ).freeze

  ABBR_DAYS_keys_strptime = ( /\A(#{Format::ABBR_DAYS_keys_or})/io ).freeze

  MONTHS_keys_strptime = ( /\A(#{Format::MONTHS_keys_or})/io ).freeze

  ABBR_MONTHS_keys_strptime = ( /\A(#{Format::ABBR_MONTHS_keys_or})/io ).freeze
  ...
  def self._strptime_i(str, fmt, e) # :nodoc:
    fmt.scan(/%([EO]?(?::{1,3}z|.))|(.)/m) do |s, c|
      ...
      if s
        case s
        when 'A', 'a'
          return unless str.sub!( Format::DAYS_keys_strptime, '') ||
            str.sub!( Format::ABBR_DAYS_keys_strptime, '')
          val = Format::DAYS[$1.downcase] || Format::ABBR_DAYS[$1.downcase]
          return unless val
          e.wday = val
        when 'B', 'b', 'h'
          return unless str.sub!( MONTHS_keys_strptime, '') ||
            str.sub!( ABBR_MONTHS_keys_strptime, '')
  ...
  ABBR_DAYS_keys_parse_day = ( /\b(#{Format::ABBR_DAYS_keys_or})[^-\d\s]*/in ).freeze

  def self._parse_day(str, e) # :nodoc:
    if str.sub!( ABBR_DAYS_keys_parse_day , ' ')
  ...
  ABBR_MONTHS_keys_eu = ( /'?(\d+)[^-\d\s]*
     \s*
     (#{Format::ABBR_MONTHS_keys_or})[^-\d\s']*
     (?:
       \s*
  ...
  def self._parse_eu(str, e) # :nodoc:
    if str.sub!( ABBR_MONTHS_keys_eu , ' ')
  ...
  ABBR_MONTHS_keys_us = ( /\b(#{Format::ABBR_MONTHS_keys_or})[^-\d\s']*
     \s*
     ('?\d+)[^-\d\s']*
     (?:
       \s*
  ...
  def self._parse_us(str, e) # :nodoc:
    if str.sub!( ABBR_MONTHS_keys_us , ' ')
  ...
  ERA_TABLE = { 'm'=>1867, 't'=>1911, 's'=>1925, 'h'=>1988 }.freeze

  def self._parse_jis(str, e) # :nodoc:
    if str.sub!(/\b([mtsh])(\d+)\.(\d+)\.(\d+)/in, ' ')
      era = ERA_TABLE[$1.downcase]
      e.year = $2.to_i + era
      e.mon = $3.to_i
      e.mday = $4.to_i
      true
  ...
  ABBR_MONTHS_keys_vms = ( /('?-?\d+)-(#{Format::ABBR_MONTHS_keys_or})[^-]*
    -('?-?\d+)/inox ).freeze

  ABBR_MONTHS_keys_vms_B = ( /\b(#{Format::ABBR_MONTHS_keys_or})[^-]*
    -('?-?\d+)(?:-('?-?\d+))?/inox ).freeze

  def self._parse_vms(str, e) # :nodoc:
    if str.sub!( ABBR_MONTHS_keys_vms, ' ')
      s3e(e, $3, Format::ABBR_MONTHS[$2.downcase], $1)
      true
    elsif str.sub!( ABBR_MONTHS_keys_vms_B , ' ')
  ...
  ABBR_MONTHS_keys_mon = ( /\b(#{Format::ABBR_MONTHS_keys_or})\S*/ino ).freeze

  def self._parse_mon(str, e) # :nodoc:
    if str.sub!( ABBR_MONTHS_keys_mon, ' ')
  ...
  def self.zone_to_diff(zone) # :nodoc:
    ...
    formatZONES = Format::ZONES
    if formatZONES.include?(zone)
      offset = formatZONES[zone]
      offset += 3600 if dst
  ...
  def self._parse_httpdate(str)
    unless str =~ /(\w\w\w), (\d\d?) (\w\w\w) (-?\d\d\d?\d?) (\d\d):(\d\d):(\d\d) GMT/
      return []
    end
    ...
    min  = $6.to_i
    sec  = $7.to_i
    return [wday, day, mon, year, hour, min, sec]
  end
  private :_parse_httpdate

  def self.httpdate(str='Mon, 01 Jan -4712 00:00:00 GMT')
    wday, day, mon, year, hour, min, sec = _parse_httpdate(str)
    unless day && mon && year
...
class DateTime < Date
  ...
  def self.httpdate(str='Mon, 01 Jan -4712 00:00:00 GMT')
    wday, day, mon, year, hour, min, sec = _parse_httpdate(str)
    unless day && mon && year && hour && min && sec
      raise ArgumentError, "invalid date"
    end
