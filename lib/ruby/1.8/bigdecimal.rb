...
class BigDecimal < Numeric
  ...
  def self.limit(val=nil)
    ...
    @@prec_limit = val if !val.equal?(nil)
    return old_limit
  end
  ...
  def div(other, precs = nil)
    if !other.kind_of?(BigDecimal)
      ...
      if precs.equal?(nil) or self.infinite?
        return BigDecimal("NaN") 
      else
        return BigDecimal("0")
      end
    elsif other.zero?
      return BigDecimal("NaN") if precs.equal?(nil) or self.zero?
      ...
      a, b, extra = reduce(self, other)
      q = precs.equal?(nil) ? (a / b).floor : (a / b)
      p = precs.equal?(nil) ? q.to_s.length : precs
  ...
  def divmod(other)
    ...
    raise TypeError if other._isString
    other = BigDecimal(other.to_s) if other._isInteger
    ...
    arr << first.to_i << second.to_f if other._isFloat
    arr << first << second
  end
 ...
 protected
  ...
  #  else
      result = (self <=> other)
      return result.equal?(nil) ? nil : result == val
  #  end
  end
