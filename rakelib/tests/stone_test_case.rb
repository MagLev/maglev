...
require File.join(File.dirname(__FILE__), "..", 'maglev_stone')
...
class StoneUnitTestCase < StoneTestCase
  def test_full_backup
    stone = MagLevStone.create(TEST_STONE_NAME)
    partial_mock_stone = flexmock(stone)

    log_number = "1313"

    # TODO: Fix this broken test case
    # startNewLog seems to return -1, expressed as the following string:
    # [18446744073709551610 sz:0 cls: 74241 SmallInteger] -1 == 0xffffffffffffffff
    # The following line is incorrect for GemStone/S 3.x
...
class StoneIntegrationTestCase < StoneTestCase
  ...
  def test_existing_stone_retrieve
    MagLevStone.create TEST_STONE_NAME
  ...
  def test_create_config_file
    ...
    stone = MagLevStone.new(TEST_STONE_NAME, GemStoneInstallation.current).create_config_file
  ...
  def test_stones_in_different_installations_uses_correct_environment
    ...
    previous = GemStoneInstallation.new('/tmp/fake_previous', current.config_directory, current.installation_extent_directory, current.base_log_directory, current.backup_directory)
    current_stone = MagLevStone.create(TEST_STONE_NAME)
    previous_stone = MagLevStone.create(fake_previous_stone_name, previous)
