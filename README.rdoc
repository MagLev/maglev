...
Install using <tt>rvm install maglev</tt>. See http://rvm.beginrescueend.com for details
of using RVM. RVM will start a MagLev server when you <tt>rvm use maglev</tt>. The server
is left running if you "rvm use <another impl>" so you'll stll be able to use <tt>maglev-ruby</tt>
if you set your $MAGLEV_HOME and $PATH appropriately.
