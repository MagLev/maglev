...
execMethBridgeTo: aMethod
  ...
  | br toSuffix toSel ch optArgsDescr argsDescrInt toNargs |
  toSel := aMethod selector .
  toSuffix :=  toSel suffixIfRubySelector .
  toSuffix = '#0*&' ifTrue:[ ^ nil "no bridge needed" ].
  ...
  toSuffix ifNotNil:[
    (ch := toSuffix at: 4) == $& ifTrue:[ 
       toNargs := toNargs - 1 .
       argsDescrInt := argsDescrInt bitOr: 16r100000 .
    ]. 
    (ch := toSuffix at: 3) == $* ifTrue:[ 
       toNargs := toNargs - 1 .
  ...
  br setTo: toSel suffix: toSuffix argsDesc: argsDescrInt primKind: 0 .
