...
class RubyLexer
  ...
  def advance  # old name  was yylex # [
    ...
    while true # [
      ...
      if (s_ch_type >= RpStringScanner::CTYPE_DIGIT) # [
        if s_ch_type.equal?( RpStringScanner::CTYPE_DIGIT) 
          return parse_number
        else
      ...
      elsif s_ch_type >= RpStringScanner::CTYPE_EOF  # ] [
        if s_ch_type.equal?( RpStringScanner::CTYPE_SIGN) #  src.scan(/[+-]/))  # [
          src.advance(1)
          if s_ch.equal?( ?+ ) 
            utype = :tUPLUS
            type = :tPLUS
 	  ...
 	  return :tRCURLY
          ...
          if eq_code.equal?( 0 ) # src.scan(/\=\=\=/) then
            ...
            src.advance(1)
            if src.peek_ahead(-2).equal?( ?\n ) and src.check_advance(/begin(?=\s)/) 
