...
I got the following performance on my "laptop class" "server":
  ...
  | Configuration        | Req/s |
  |----------------------+-------+
  | Lighttpd + 1 FastCGI | 789   |
  | Lighttpd + 4 FastCGI | 789   |
...
Some targets:
1. rake kill!      To kill an entire experiment (HTTPd and VMs)
2. rake vm:kill!   To kill just the Ruby VMs on ports 3000-3003
3. rake http:kill! To kill just the HTTPd on port 3333
