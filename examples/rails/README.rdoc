...
pages using a patched version of Rails 3.0.0.rc.  The <tt>Rakefile</tt>
...
Rails installs from gems, but needs a couple of patches.
...
The new app is correctly built, but there are a few items that need to be
...
3. Edit <tt>myapp/config/database.yml</tt>
   Change the adapter from 'mysql2' to 'mysql'.  Depending on how your
   MySQL is configured, you may need to add something like the following to
   your configurations:
