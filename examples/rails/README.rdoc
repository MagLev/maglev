...
To start the maglev server:

  $ cd $MAGLEV_HOME
  $ rake maglev:start  # or: ./bin/maglev start

To checkout the patched rails and install the gems:

  $ cd $MAGLEV_HOME/examples/rails
  $ rake checkout      # Requires a working copy of git
  $ rake gems
...
The changes to the rails code are rather minimal.  To get a good look:
  ...
  or browse to:
  http://github.com/MagLev/rails/compare/master...maglev-beta4
