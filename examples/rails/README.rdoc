...
ActiveRecord now seems to work well enough that we no longer patch it.
...
==== 2. Create new rails app: Works, but must patch app files.

Run:
  $ $MAGLEV_HOME/bin/rails new myapp --database=mysql
  $ rake patch:app[myapp]
  <edit myapp/Gemfile>

The new app is correctly built, but there are two items that need to be
upated in the new app.

1. You should edit <tt>myapp/Gemfile</tt> and change the <tt>gem
   'mysql'</tt> line to be:
     gem 'ruby-mysql', :require => 'mysql'
2. Workaround for  {Trac 757}[https://magtrac.gemstone.com/ticket/757]:
   Run "rake patch:app[myapp]" to patch <tt>myapp/config/boot.rb</tt>.
