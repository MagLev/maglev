...
Currently, ActiveRecord does not work.  We are working getting a worthy db
driver. See the example for how to configure around loading of
ActiveRecord. 
...
The second pause occurs when you hit the first dynamic page.  Both of these
pauses are due to MagLev parsing and compiling Ruby code.  Subsequent
dynamic page loads should be much quicker, as MagLev only has to compile
the code at startup.
