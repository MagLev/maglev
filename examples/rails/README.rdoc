...
2. Install Rails, patch it and install other gems
...
Currently, ActiveRecord does not work, and we are working on fixes.  In the
meantime, you must configure around ActiveRecord.  This has already been
done for the +myapp+ in this directory.  To configure around ActiveRecord
in another app, edit <tt>yourapp/config/application.rb</tt> and change:
...
The second and thirs pauses occur when you hit the first static and first
...
<tt>-MallIvsDynamic</tt> flag to <tt>maglev-ruby</tt> (you can set it on
the command line, or in the <tt>MAGLEV_OPTS</tt> env variable).
...
* Edited <tt>myapp/Gemfile</tt> to use ruby-mysql gem.  Change <tt>gem
  'mysql'</tt> to <tt>gem 'ruby-mysql', :require => 'mysql'</tt>.
...
* <tt>myapp/config/application.rb</tt> was edited to replace <tt>require
  ...
  +ActiveRecord+. ActiveRecord doesn't currently load under MagLev.
...
* In order to get the "About your application's environment" link on the
  ...
  myapp/config/routes.rb.  The demo app has a route added, so the sample
  URL should work. See
  {Trac 766}[https://magtrac.gemstone.com/ticket/766].
...
Tutorial}[http://guides.rails.info/getting_started.html], and are what we
are using to drive forward on getting rails working.

1. Install rails: <tt>maglev-gem install rails --pre</tt> (works with patch)
2. Create new rails app: <tt>$MAGLEV_HOME/bin/rails new blog</tt> (works)
3. Run +bundle+: <tt>$MAGLEV_HOME/bin/bundle install</tt> (works)
4. Create the database: <tt>$MAGLEV_HOME/bin/rake db:create</tt> (broken)
...
8. Generate new controller: <tt>$MAGLEV_HOME/bin/rails generate controller
   home index</tt> (works with workaround)
9. Running a migrations: <tt>$MAGLEV_HOME/bin/rake db:migrate</tt>
   (untested)
10. Running the rails console: <tt>$MAGLEV_HOME/bin/rails console</tt>
    (untested)

The following sections explain details and current status for each step above.

==== 1. Install rails: Works with patch.

Rails installs from gems, but needs one patch.  To install the rails gems,
you can (A) run <tt>rake init</tt>, or can do the following:
...
==== 2. Create new rails app: Works.

Run:
  $ $MAGLEV_HOME/bin/rails new myapp

You should edit <tt>myapp/Gemfile</tt> and change the <tt>gem 'mysql'</tt>
line to be:
...
==== 4. Create the database: Broken.

The next step is to run:
  $ $MAGLEV_HOME/bin/rake db:create

But, the rake task does not even exist, since we are not requiring
<tt>active_record/railtie</tt>. If you load active record, then MagLev gets
the following error:

  rake aborted!
  NoMethodError: undefined method `attribute_method_suffix' for

Once we resolve the ActiveRecord problem, we expect this task to start
working.
...
with dynamic routes, unless some other route is defined in
<tt>route.rb</tt>.

==== 8. Generate new controller: Works with patch.

<tt>myapp/config/boot.rb</tt> has been patched to work around
{Trac 757}[https://magtrac.gemstone.com/ticket/757].   With the patch, you
can run:
  $ $MAGLEV_HOME/bin/rails generate controller home index

and generate the expected files.

==== 9. Running a migrations: Untested.

Running <tt>$MAGLEV_HOME/bin/rake db:migrate</tt> is untested.
