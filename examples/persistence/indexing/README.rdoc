...
== Fixed vs Dynamic inst vars

MagLev now defaults to all instvars dynamic.  The indexing subsystem has
not yet been updated to work with dynamic inst vars, so we must declare
them as fixed.  See https://magtrac.gemstone.com/ticket/844
