...
This directory contains an implementation of a KD-Tree.  A derivative class,
Tree2D (k=2), is shown in this example using the Google Maps API as the GUI.  
This is a binary tree that can be used to search for 2D data, e.g.,
nodes with a latitude and longitude.  In this demo, only the closest zipcode 
(i.e. k = 1 when invoking nearest_k()) for a selected latitude and longitude 
will be displayed along with the location of the selection. 
