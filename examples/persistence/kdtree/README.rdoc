...
This directory contains an implementation of a {K-Dimensional
Tree}[http://en.wikipedia.org/wiki/Kd-tree] (KD-Tree).  A derivative class,
Tree2D (k=2), is shown in this example.  This is a binary tree that can be
used to search for 2D data, i.e., nodes with a latitude and longitude.
...
Postal Database), and will find the nearest N zip codes to the lat/lon.

1. Make sure the maglev server has been started:

  $ cd $MAGLEV_HOME
  $ rake maglev:start

2. Make sure Sinatra has been installed:
...
You can run MagLev with profiling turned on and take a look at the
performance details (see which methods are called the most, and from where,
etc.):
...
Note: Changes were made after the collection of performance results.
Specifically, the 2D tree was transformed into a generalized K-dimension
tree.  This made no noticable impact on new benchmarks, so the old ones
(which were used in http://maglevity.wordpress.com/2009/12/17/kd-trees-and-maglev/)
have been kept intact as follows.
...
The table below lists the time to run 1,000 queries for the nearest 100
nodes in Maglev, MRI 1.8.6, MRI 1.9.2 and MRI 1.8.6 + KDTree Gem (C
implementation), fastest to slowest:
...
Here are the times to create and commit the data in MagLev:
...
The time to iterate all nodes and the time to run 1,000 queries for the
nearest 100 nodes in the previously committed tree in MagLev:
...
The data file that has the US postal codes is provided by
http://GeoNames.org/ under a Creative Commons Attribution 3.0 License.  See
<tt>etc/readme.txt</tt>[link:files/examples/persistence/kdtree/etc/readme_txt.html]
for details.
