...
The KDTree[link:examples/persistence/kdtree/KDTree-README_rdoc.html] exmple
contains a pure ruby implementation of a KD-Tree, a data structure that
allows fast nearest-neighbor look-ups in a given dimension.  In this
example, we use the Google Maps API as a GUI to display the closest zip
codes to a given location selected on the map (a KD-Tree where K=2, since
