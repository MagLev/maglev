...
that means the gem is discussed more fully below:
  ...
  | rack      | ~> 1.2  | Works               |
  | rails*    | ~> 3.0  | Works with patches  |
  | rake      | ~> 0.8  | Works               |
  | sinatra*  | ~> 1.1  | Works, with caveats |
...
Rails works, but you need to apply a couple of patches after you've gem
installed it.  See details in examples/rails/README.rdoc.
